...
- **REQ-53:** End the first mission when the ship's hull and ammo are full, fuel exceeds 80, and no mining drones are hunting; display "well done" and wait for the player's click before continuing.
- **REQ-54:** After the first mission, when the player flies into space they have not previously visited, stop spawning new world objects there so the area is empty; entering such uncharted space starts the second mission.
- **REQ-55:** Display "Mission: traverse empty space" with "click to continue" for the second mission and wait for the player's click; on click, restart the game (re-running the first mission).
...
- **REQ-23:** Display a ship-centered scrolling minimap of camera-explored space, discovered regular asteroids, discovered fuel, repair, and ammo containers, discovered mining drones, and discovered massive asteroids represented by their irregular outlines.
